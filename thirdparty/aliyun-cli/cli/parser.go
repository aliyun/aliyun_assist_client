...
func (p *Parser) readNext() (arg string, flag *Flag, more bool, err error) {
	...
	p.current++
	more = true
	// 为了跟cpp版的acs-plugin-manager一致，"--params -v"中的 -v 应该被识别成要传递给插件的参数而不是给acs-plugin-manager的参数，增加了一下几行代码
	if p.currentFlag != nil && p.currentFlag.AssignedMode == AssignedOnce && p.currentFlag.needValue() {
		p.currentFlag.assign(s)
		p.currentFlag = nil
		return
	}
