...
func (*Fingerprint) Name() string {
	return MachineIdentifierName
}

// Refresh generate fingerprint regardless of whether
// a hardware hash already exists.
func (*Fingerprint) Refresh() (string, error) {
	return generateFingerprint(true)
}

// Generate generate fingerprint, but if hardware hash already exists
// and it is similar enough to current hardware the fingerprint in hardware hash
// will be returned.
func (*Fingerprint) Generate() (string, error) {
	return generateFingerprint(false)
}

// Cleanup would update hardwareInfo and hope this instance will be recognized
// when next registration
func (*Fingerprint) Cleanup() error {
	if hardwareInfo, err := ReadHardwareInfo(); err == nil {
		if currentHardwareInfo, err := CurrentHwHash(); err == nil {
			SaveHardwareInfo(hardwareInfo.Fingerprint, currentHardwareInfo)
		}
	}
