...
func Start(winptyDllFilePath, cmdLine string, window_size_cols, window_size_rows uint32, winptyFlag int32, env []string, pwd string) (*WinPTY, error) {
	...
	if err := winpty.spawnProcess(cmdLine, env, pwd); err != nil {
		return nil, err
	}
...
func (winpty *WinPTY) spawnProcess(cmdLine string, env []string, pwd string) (err error) {
	var errorPtr uintptr
	defer winpty_error_free.Call(errorPtr)

	cmdLineUTF16Ptr, err := syscall.UTF16PtrFromString(cmdLine)
	if err != nil {
		return fmt.Errorf("Failed to convert cmd to pointer. %w", err)
	}

	var envBlock *uint16
	if len(env) > 0 {
		if envBlock, err = createEnvBlock(env); err != nil {
			return fmt.Errorf("Failed to create env block. %w", err)
		}
	}

	var pwdUTF16Ptr *uint16
	if pwd != "" {
		if pwdUTF16Ptr, err = syscall.UTF16PtrFromString(pwd); err != nil {
			return fmt.Errorf("Failed to convert pwd to pointer. %w", err)
		}
	}
	...
	spawnConfig, _, lastErr := winpty_spawn_config_new.Call(
		...
		uintptr(unsafe.Pointer(pwdUTF16Ptr)),
