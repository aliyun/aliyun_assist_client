...
func StartSelfKillMon() {
	...
	_perf = StartPerfmon(os.Getpid(), 1, func(cpuUsage float64, memory uint64, threads uint64) {
		if !_taskFactory.IsAnyTaskRunning() && !update.IsCriticalActionRunning() { //没有任务执行时才监控性能
